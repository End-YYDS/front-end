...
import { MagnifyingGlassIcon, Square2StackIcon, LinkIcon, InboxArrowDownIcon, ArrowPathIcon, TrashIcon } from '@heroicons/react/24/outline';
import { Tree } from '@assets';

const SearchInput = ({ placeholder, onChange, className }: { placeholder?: string, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void, className?: string }) => (
    <input
        type="text"
...
const Button = ({ children, onClick, color, className }: { children: React.ReactNode, onClick: () => void, color: string, className?: string }) => (
    <button
        onClick={onClick}
...
export default function SoftwarePackage() {
    ...
    return (
        <div className="p-0">
            <div className="w-full h-16 bg-head_gray flex justify-center items-center mb-6">
                <h1 className="text-xl font-bold">Software Package</h1>
            </div>
            <PackageCard title="Installed Packages">
                <div className='flex'>
                    <div className="w-44 p-2 bg-head_gray flex items-center">
